...
#' @description The function generates discretized probability distributions of various losses arising from a cyber system.
#' @param th a numeric vector, of which the length equals the number of vulnerabilities. Elements in the vector range from 0 to 1. `th` (Vector \eqn{\theta}) represents the scaling of loss random variables when cybersecurity controls are present. If losses are unmitigated, then `th` should be a vector of one's.
#' @param cyra.params a list of S3 class cyraParams, generated by function [prepareCyraParameters()].
#' @param scale.fun a function that returns the new parameters of a probability distribution of a random variable after it is scaled. For more details, see built-in functions [scaleLogNormal()] (default) and [scaleWeibull()]. A compatible customer function can also be provided.
#'
#' @return This function returns a list of discretized probability distributions that are represented by quantile-probability tables. Items in the list are as follows,
#' * X_dist: the distribution of \eqn{X_{ijk}}, the loss random variable on each attack path defined by a threat-vulnerability-asset tuple.
#' * Zik_dist: the distribution of \eqn{Z_{ik}}, the loss random variable associated with each threat-asset pair.
#' * Zi_dist: the distribution of \eqn{Z_i}, the loss random variable associated with each threat.
#' * L_dist: the distribution of \eqn{L}, the random variable of the total loss in one incident.
#' * S_dist: the distribution of \eqn{S}, the random variable of the aggregate loss over a period.
#' * Sik_dist: the distribution of \eqn{S_{ik}}, the random variable associated with the aggregate loss on each threat-asset pair over a period.
#'
